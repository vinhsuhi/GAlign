    ...
    def _build_features(self, p_change_feats=None):
        ...
        if os.path.isfile(self.networkx_dir+"/feats.npy"):
            ...
            if p_change_feats is not None:
                for k in range(features_ori.shape[0]):
                    p = np.random.rand()
                    if(p<p_change_feats):
                        new_feature_k = np.zeros(len(features_ori[k]))
                        new_index = np.random.randint(0,len(features_ori[k]),int(features_ori[k].sum()))
                        new_feature_k[new_index]+=1
                        features_ori[k] = new_feature_k
